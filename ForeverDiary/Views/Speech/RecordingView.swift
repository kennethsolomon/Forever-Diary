...
struct RecordingView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            // Top row: language pill + time remaining
            HStack {
                languagePill
        ...
        NavigationStack {
            List {
                if speechService.engineChoice == .whisperKit {
                    Section {
                        Button {
                            speechService.languageIdentifier = "auto"
                            dismiss()
                        } label: {
                            HStack {
                                Text("Auto-detect")
                                ...
                                Spacer()
                                if speechService.languageIdentifier == "auto" {
                ...
                Section {
                    ForEach(SpeechService.supportedLocales, id: \.identifier) { locale in
                        Button {
                            speechService.languageIdentifier = locale.identifier
                            dismiss()
                        } label: {
                            HStack {
                                Text(locale.localizedString(forIdentifier: locale.identifier) ?? locale.identifier)
                                    ...
                                    .foregroundStyle(Color("textPrimary"))
                                Spacer()
                                if speechService.languageIdentifier == locale.identifier {
                                    Image(systemName: "checkmark")
                                        .foregroundStyle(Color("accentBright"))
                                }
                            }
